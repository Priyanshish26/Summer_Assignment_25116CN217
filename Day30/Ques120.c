...
int main(){
    ...
    printf("%-20s%s\n%-20s%s\n%-20s%s\n%-20s\n", "1: Add Student","2: Sort Student", "3: Display Student", "4: Search Student", "5: Update Student", "6: Highest Marks", "7: EXIT");
    for(int i=0; i<width ; i++){
        printf("*");
    }
...
void Highest(stu S1[], int n)
{
    ...
    printf("Top Student: %s (%f marks)\n", S1[index].name, S1[index].percentage);
}
